...
HTTP_STATUS=$(sudo /usr/bin/curl -s --unix-socket /var/run/docker.sock -w "%{http_code}" -H "Content-Type: application/json" -d @/tmp/payload_download_$1.json http:/v1.37/exec/${EXEC_ID}/start | tr '\0' '\n')
echo "${HTTP_STATUS}"
