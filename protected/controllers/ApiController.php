...
class ApiController extends Controller
{
    ...
    public function actionDataset()
    {
        ...
        // needed in order to remove the empty line (TODO: find the file with an empty line at the top)
        ob_start();
        $image = $model->image;
        ob_get_clean();
    ...
    public function actionList()
    {
        ...
        $datasets = Dataset::model()->findByStatusAndDate($status, $startDate, $endDate);
    ...
    public function actionFile()
    {
        ...
        try {
            if ($id) {
                ...
                if (!$model) {
                    $this->_sendResponse(404, sprintf('No items found for file id <b>%s</b>', $id));
                }
            } else {
                $model = Dataset::model()->findByAttributes(array('identifier' => $doi, 'upload_status' => $status));

                if (!$model) {
                    $this->_sendResponse(404, sprintf('No items where found for file doi <b>%s</b>', $doi));
                }
            }
    ...
    public function actionSample()
    {
        ...
        try {
            if ($id) {
                $model = Dataset::model()->findByAttributes(array('id' => $id, 'upload_status' => $status));
                if (!$model) {
                    $this->_sendResponse(404, sprintf('No items where found for sample id <b>%s</b>', $id));
                }
            } else {
                $model = Dataset::model()->findByAttributes(array('identifier' => $doi, 'upload_status' => $status));
                if (!$model) {
                    $this->_sendResponse(404, sprintf('No items where found for sample doi <b>%s</b>', $doi));
                }
            }
        ...
        try {
            if ($keyword) {
                if (strpos($keyword, ':')) {
                    ...
                    $columns = array_keys(Dataset::model()->tableSchema->columns);

                    if (!in_array($pieces[0], $columns)) {
                        $this->_sendResponse(500, 'A problem occurred');
                    }

                    $criteria = new CDbCriteria();
                    $criteria->addSearchCondition($pieces[0], $pieces[1]);
                    $models = Dataset::model()->findAll($criteria);
                ...
                switch ($result) {
                    case 'dataset':
                        ...
                        $this->renderPartial('keywordalldataset', array('datasetids' => $datasets));
                        break;
                    case 'sample':
                        if (!$samples) {
                            $this->_sendResponse(
                    ...
                    case 'file':
                        ...
                        $this->renderPartial('keywordallfile', array('fileids' => $files));
                        break;
                    default:
            ...
            if ($taxno) {
                ...
                if (count($rows) < 1) {
                    $this->_sendResponse(404, sprintf('No items where found for taxno <b>%s</b>', $taxno));
                }
    ...
    private function _getStatusCodeMessage(int $status): string
    {
        ...
        //TODO: no statud code if not found??
    ...
    * @return  string $body
    */
    private function getDOIBannerBody(): string
    {
