...
class m220126_062259_update_image_column_location_project_tab extends CDbMigration
{
	public function safeUp()
	{
        ...
        Yii::app()->db->createCommand("update project set name = 'ligea' where name = 'ligea: a comprehensive database of human gene fusion events';")->execute();
        Yii::app()->db->createCommand("update project set name = replace( name, ',', '');")->execute();
        Yii::app()->db->createCommand("update project set name = replace( name, ' - ', ' ');")->execute();
