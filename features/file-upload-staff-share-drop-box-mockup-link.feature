Feature:
	...
	So that I can share access to privately uploaded files of a dataset with select curators

Scenario: The button appears in a dataset row from the admin list when status is DataAvailableForReview
...
Scenario: The button does not appear in a dataset row from the admin list when status is not DataAvailableForReview
	Given I am on the admin page
