...
if ! [ -s ./.secrets ];then
    ...
    cat .project_vars.json | jq --arg ENVIRONMENT $GIGADB_ENV -r '.[] | select(.environment_scope == "*" or .environment_scope == "dev" ) | select(.key | test("private_key|tlsauth|ca|pem|cert|^hetzner_storage_box_";"i") | not ) |.key + "=" + .value' > .project_var
...
VARS='$MAILCHIMP_API_KEY:$MAILCHIMP_LIST_ID:$ANALYTICS_CLIENT_EMAIL:$ANALYTICS_CLIENT_ID:$ANALYTICS_KEYFILE_PATH:$HOME_URL:$SERVER_EMAIL:$RECAPTCHA_PUBLICKEY:$RECAPTCHA_PRIVATEKEY:$GOOGLE_ANALYTICS_PROFILE:$MDS_DOI_URL:$MDS_METADATA_URL:$MDS_USERNAME:$MDS_PASSWORD:$MDS_PREFIX:$SEARCH_RESULT_LIMIT:$HASH_SECRET_KEY:$FTP_CONNECTION_URL'
envsubst $VARS < $SOURCE > $TARGET
...
VARS='$OPAUTH_SECURITY_SALT:$FACEBOOK_APP_ID:$FACEBOOK_APP_SECRET:$LINKEDIN_API_KEY:$LINKEDIN_SECRET_KEY:$GOOGLE_CLIENT_ID:$GOOGLE_SECRET:$TWITTER_KEY:$TWITTER_SECRET:$ORCID_CLIENT_ID:$ORCID_CLIENT_SECRET:$ORCID_CLIENT_ENVIRONMENT:$FTP_CONNECTION_URL:$APP_VERSION:$FFLAG_API_URL:$FFLAG_ID:$GIGADB_ENV:$SECRET_KEY_EMAIL:$COOKIE_VALIDATION_KEY'
envsubst $VARS < $SOURCE > $TARGET
...
then
  ...
  VARS='$SERVER_EMAIL_SMTP_HOST:$SERVER_EMAIL_SMTP_PORT:$SERVER_EMAIL:$SERVER_EMAIL_USERNAME:$SERVER_EMAIL_PASSWORD:$AWS_ACCESS_KEY_ID:$AWS_SECRET_ACCESS_KEY:$COOKIE_VALIDATION_KEY'
  envsubst $VARS < $SOURCE > $TARGET
else
...
VARS='$SERVER_EMAIL_SMTP_HOST:$SERVER_EMAIL_SMTP_PORT:$SERVER_EMAIL:$SERVER_EMAIL_USERNAME:$SERVER_EMAIL_PASSWORD:$AWS_ACCESS_KEY_ID:$AWS_SECRET_ACCESS_KEY'
envsubst $VARS < $SOURCE > $TARGET
