...
if [[ $? -eq 0 ]];then
  ...
  $DOCKER_COMPOSE exec -T web /usr/bin/curl --show-error --silent --output /dev/null  \
  ...
  http_code_get_fullchain=$($DOCKER_COMPOSE exec -T web /usr/bin/curl --HEAD --url "$CI_PROJECT_URL/variables/tls_fullchain_pem?filter%5benvironment_scope%5d=$GIGADB_ENV" \
    --header "PRIVATE-TOKEN: $GITLAB_PRIVATE_TOKEN" | head -1 | cut -d" " -f 2 \
)
  http_code_get_private=$($DOCKER_COMPOSE exec -T web /usr/bin/curl --HEAD --url "$CI_PROJECT_URL/variables/tls_privkey_pem?filter%5benvironment_scope%5d=$GIGADB_ENV" \
    --header "PRIVATE-TOKEN: $GITLAB_PRIVATE_TOKEN" | head -1 | cut -d" " -f 2 \
  )
  http_code_get_chain=$($DOCKER_COMPOSE exec -T web /usr/bin/curl --HEAD --url "$CI_PROJECT_URL/variables/tls_chain_pem?filter%5benvironment_scope%5d=$GIGADB_ENV" \
    --header "PRIVATE-TOKEN: $GITLAB_PRIVATE_TOKEN" | head -1 | cut -d" " -f 2 \
  )
  ...
  if [[ $http_code_get_fullchain -eq 200 && $http_code_get_private -eq 200 && $http_code_get_chain -eq 200 ]];then
    ...
    $DOCKER_COMPOSE exec -T web /usr/bin/curl --show-error --silent \
