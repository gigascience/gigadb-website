...
$DOCKER_COMPOSE run --rm test bash -c "psql -h $REMOTE_DOCKER_HOST -U gigadb -c 'create database gigadb_testdata'" || true
...
$DOCKER_COMPOSE run --rm test bash -c "PGPASSWORD=$GIGADB_PASSWORD pg_dump --no-owner -U gigadb -h $REMOTE_DOCKER_HOST -p 5432 -F custom -d gigadb_testdata -f /var/www/sql/gigadb_testdata.pgdmp"
