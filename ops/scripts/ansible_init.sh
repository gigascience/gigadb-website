...
hetzner_storage_box_user=$(curl -s --header "PRIVATE-TOKEN: $GITLAB_PRIVATE_TOKEN" "$FORK_VARIABLES_URL/hetzner_storage_box_user" | jq -r .value)
hetzner_storage_box_hostname=$(curl -s --header "PRIVATE-TOKEN: $GITLAB_PRIVATE_TOKEN" "$FORK_VARIABLES_URL/hetzner_storage_box_hostname" | jq -r .value)
