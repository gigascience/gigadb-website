...
casper.test.begin('Removing all keywords', 5, function(test) {
    ...
    // casper.waitForUrl('http://127.0.0.1:9170/dataset/100002', function datasetViewIsLoaded() {
	...
	casper.waitForSelector('h3.dataset-title', function datasetViewIsLoaded() {
		test.assertTextDoesntExist('Keywords:','The dataset view should not contain "Keywords:" when no keywords');
	});
...
casper.test.begin('Adding new keywords', 2, function(test) {
...
// tear-down
casper.test.begin('Tear-down: Removing  keywords, Logout', 2, function(test) {
