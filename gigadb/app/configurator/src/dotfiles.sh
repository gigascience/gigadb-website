...
function makeDotSecrets () {
  ...
  if ! [ -s "$mdsBaseDir/.secrets" ];then
      ...
      cat "$mdsBaseDir/.project_vars.json" | jq --arg ENVIRONMENT $GIGADB_ENV -r '.[] | select( .environment_scope == $ENVIRONMENT or .environment_scope == "*") | select(.key | test("private_key|tlsauth|ca|pem|cert";"i") | not ) |.key + "=" + .value' > "$mdsBaseDir/.project_var"
