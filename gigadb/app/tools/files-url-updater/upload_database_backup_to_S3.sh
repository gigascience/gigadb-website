...
export PGPASSWORD=$DB_PG_PASSWORD; pg_dump --host=$DB_PG_HOST -p 5432 --username=$DB_PG_USER --clean --create --schema=public --no-privileges --no-tablespaces --dbname=gigadb --file=backups/gigadb_${GITLAB_PROJECT_STRING}_${GIGADB_ENVIRONMENT}_${latest}.backup
