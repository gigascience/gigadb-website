...
psql -U $DB_PG_USER -d $DB_PG_DATABASE -h $DB_PG_HOST -p 5432 -c "select count(*) from dataset"
psql -U $DB_PG_USER -d $DB_PG_DATABASE -h $DB_PG_HOST -p 5432 -c "select identifier, publication_date from dataset where publication_date is not null order by publication_date desc limit 1"
