...
if ! [ -f  ./.rclone.conf ];then
  ...
  sed -i'.bak' "s/wasabi_access_key_id =/access_key_id = $key/" .rclone.conf
  sed -i'.bak' "s/wasabi_secret_access_key =/secret_access_key = $secret/" .rclone.conf
  sed -i'.bak' "s/wasabi_region =/region = $region/" .rclone.conf
  rm .rclone.conf.bak
fi