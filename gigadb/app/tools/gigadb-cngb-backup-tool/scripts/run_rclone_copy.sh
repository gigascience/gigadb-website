...
#time rclone --config=.rclone.conf --dry-run copy --checksum --log-level DEBUG --log-file=tests/test-logs/test_rclone_copy_interrupt_network.log /Volumes/kencho wasabi:test-gigadb-datasets/test-ken-20221010/NA12878_delete_source
...
time rclone --config=.rclone.conf copy --checksum --log-level DEBUG --retries 3 --low-level-retries 5 --stats 1s --log-file=tests/test-logs/test_rclone_copy_interrupt_network.log /Volumes/kencho wasabi:test-gigadb-datasets/test-ken-20221010/NA12878_delete_source
echo $? > tests/test-logs/test_rclone_status_"$?".log