...
describe('AttributeSpecifier', function () {
	...
	it('should have a named form for adding a new attribute', function () {
		expect(
			this.renderedComponent.find('form button#add-new-attribute').exists()
		).toBe(true)
	})
	it('should have text field for name, value and unit', function () {
		expect(
			this.renderedComponent.find('input[type="text"][name="name"]').exists()
		).toBe(true)
		expect(
			this.renderedComponent.find('input[type="text"][name="value"]').exists()
		).toBe(true)
		expect(
			this.renderedComponent.find('input[type="text"][name="unit"]').exists()
		).toBe(true)
	})
	it('should have an Add attribute button', function () {
		expect(
			this.renderedComponent.find('button#add-new-attribute').exists()
		).toBe(true)
		expect(
			this.renderedComponent.find('button#add-new-attribute').text()
		).toMatch('Add')
	})
	...
	it('should display a table header', function () {
		...
		return wrapper.vm.$nextTick().then(function () {
			...
			expect(wrapper.find('table thead tr th:nth-child(2)').text()).toBe(
				'Value'
			)
			expect(wrapper.find('table thead tr th:nth-child(3)').text()).toBe('Unit')
			expect(wrapper.find('table thead tr th:nth-child(4)').text()).toBe(
				'Actions'
			)
		})
	})
	...
	it('should display a table of existing attributes', function () {
		...
		return wrapper.vm.$nextTick().then(function () {
			expect(
				wrapper.find('table tbody tr:first-child td:first-child').text()
			).toBe('Luminosity')
			expect(
				wrapper.find('table tbody tr:first-child td:nth-child(2)').text()
			).toBe('400')
			expect(
				wrapper.find('table tbody tr:first-child td:nth-child(3)').text()
			).toBe('Lux')
			expect(
				wrapper.find('table tbody tr:nth-child(2) td:first-child').text()
			).toBe('Contrast')
			expect(
				wrapper.find('table tbody tr:nth-child(2) td:nth-child(2)').text()
			).toBe('3000')
			expect(
				wrapper.find('table tbody tr:nth-child(2) td:nth-child(3)').text()
			).toBe('Nits')
	...
	it('should display delete button next to each attributes', function () {
		...
		return wrapper.vm.$nextTick().then(function () {
			expect(
				wrapper
					.find(
						'table tbody tr:first-child td:last-child button[aria-label^="Remove"]'
					)
					.exists()
			).toBe(true)
			expect(
				wrapper
					.find(
						'table tbody tr:last-child td:last-child button[aria-label^="Remove"]'
					)
					.exists()
			).toBe(true)
		})
